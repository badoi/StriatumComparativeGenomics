...
obj = obj[,obj$Region %in% c('NAcc', 'NAcc/OT')]
...
markers_df = lapply(clusters, function(x){
   df = obj %>% 
     FindConservedMarkers(ident.1 = x, grouping.var = 'Project', assay = "SCT", 
                          slot = "data", min.cells.group = 3, verbose = TRUE, 
                          logfc.threshold = .01, min.pct = .01, recorrect_umi = FALSE,
...
markers_df3 = markers_df %>% 
  mutate(max_pfdr = p.adjust(max_pval,'fdr')) %>% 
  rowwise() %>% 
  mutate(avg_log2FC = mean(c(`Tran et al._avg_log2FC`, 
...
here(DATADIR,'rdas/conserved_striatal_neuron_differential_markers_IC.rds') %>% 
  saveRDS(object = markers_df3)

dir.create(here(DATADIR,'tables'))
here(DATADIR,'tables/conserved_striatal_neuron_marker_genes_IC.xlsx') %>% 
