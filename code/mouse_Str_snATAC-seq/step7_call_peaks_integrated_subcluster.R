...
proj <- addGroupCoverages(ArchRProj = proj, groupBy = "integrated_subclusters", 
                          minCells = 100, maxCells = 500, maxFragments = 25 * 10^6,
                          minReplicates = 2, maxReplicates = 12, force = T)
proj = saveArchRProject(ArchRProj = proj)

## call peaks w/ macs2, using reproducibility of peaks more than 
proj = proj %>% 
  addReproduciblePeakSet(groupBy = "integrated_subclusters", minCells = 100, force = T, 
...
# label the summit and peak name using mm10 coordinates
...
narrowPeak_mm10_fn = paste0(PEAKDIR2, paste0(LABEL, '.', names(peakList), 
                                             '.mm10.narrowPeak.gz'))
...
## call peaks on the subclustered cells
for (varFeat in varFeats){
  ...
  proj <- addHarmony(proj, reducedDims = iterLSIName,
                     name = HarmonyName, groupBy = c("Sample","RegionName"), 
                     lambda = rep(1, 2),
                     force = TRUE)
  #clustering
