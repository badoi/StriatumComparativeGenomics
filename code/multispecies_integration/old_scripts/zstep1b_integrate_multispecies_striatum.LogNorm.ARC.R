...
obj_list <- lapply(X = obj_list, FUN = ScaleData, features = features, verbose = F)
obj_list <- lapply(X = obj_list, FUN = RunPCA, features = features, verbose = F)
pryr::mem_used()
gc()

# Zeng et al. & Siletti et al. as reference
reference = c(4, 11) 
names(obj_list)[reference]

## integrate normalized objects
anchors <- FindIntegrationAnchors(obj_list, normalization.method = "LogNormalize",
                                  reference = reference,
                                  reduction = "rpca", k.anchor = 25,
                                  anchor.features = features, dims = 1:20)

## save the anchors
anchors_fn = here(TEMPDIR, 'anchors.LogNorm.rds')
anchors_fn2 = here(DATADIR, 'rdas/tmp', 'anchors.LogNorm.rds')
saveRDS(anchors, anchors_fn)
paste('rsync -Paq', anchors_fn, anchors_fn2) %>% system()
...
combined = combined %>% ScaleData(verbose = F) %>% 
...
proj_lvl = c('Siletti et al.', 'Tran et al.', 'Phan et al.', 'Li et al.',
             'Corces et al.', 'HeKleyman et al.', 'Chiou et al.', 'Zeng et al.', 
             'Stanley et al.',  'Saunders et al.', 'Chen et al.', 'Zu et al.',  
             'Phillips et al.', 'Savell et al.')
combined$Project = factor(combined$Project, proj_lvl)

levels(combined@meta.data$Project) = levels(combined$Project) %>%
  str_replace_all('HeK', 'He, K')
table(combined$Project)

rep_lvl = combined[[]] %>% arrange(Project, Replicate) %>% 
  pull(Replicate) %>% unique()
combined$Replicate = factor(combined$Replicate, rep_lvl)

## make original cell type levels
cell_lvl = combined[[]] %>% arrange(Project, Publication.Celltype) %>% 
  pull(Publication.Celltype) %>% unique()
combined$Publication.Celltype = 
  factor(combined$Publication.Celltype, cell_lvl)

## save the harmonized object
combined_fn = here(TEMPDIR, 'harmonized.LogNorm.rds')
combined_fn2 = here(DATADIR, 'rdas/tmp', 'harmonized.LogNorm.rds')
saveRDS(combined, combined_fn)
paste('rsync -Paq', combined_fn, combined_fn2) %>% system()
gc()
...
pdf(here::here(PLOTDIR, 'Replicates_plots_bySpecies_clusters.umap.pdf'), 
    width = 11, height = 5)
DimPlot(combined, reduction = "umap", split.by = "Species", 
        cols = ArchR::paletteDiscrete(combined$seurat_clusters),
        label = T, raster = F,
...
cols2 = ArchR::paletteDiscrete(combined$Publication.Celltype)
names(cols2) = names(cols2)[sample(length(cols2))]
pdf(here::here(PLOTDIR, 'Replicates_plots_byProject_origLabel.umap.pdf'), 
    width = 20, height = 7)
DimPlot(combined, reduction = "umap", split.by = c("Project"), cols = cols2,
        label = T, raster = F,
...
table(is.na(combined$integrated_clusters), combined$Project)
table(is.na(combined$integrated_clusters), combined$seurat_clusters)
table(is.na(combined$integrated_subclusters), combined$seurat_clusters)

## make the plots
pdf(here::here(PLOTDIR, 'Replicates_plots_bySpecies_mainCluster.umap.pdf'), 
    width = 8, height = 5)
DimPlot(combined, reduction = "umap", split.by = "Species", 
        cols = ArchR::paletteDiscrete(combined$integrated_clusters),
...
combined@meta.data$integrated_subclusters = 
  factor(combined$integrated_subclusters, names(cols))

pdf(here::here(PLOTDIR, 'Replicates_plots_bySpecies_subCluster.umap.pdf'), 
    width = 8, height = 5)
DimPlot(combined, reduction = "umap", split.by = "Species", raster = F,
...
SaveSeuratRds(combined, save_fn)
