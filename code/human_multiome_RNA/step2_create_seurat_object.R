...
dataList <- mclapply(STARsolo_fn, Read10X, strip.suffix = F, mc.cores = 4)

dataList %>% lapply(colnames) %>% lapply(duplicated) %>% sapply(sum)
...
objList = lapply(objList, NormalizeData) %>% lapply(FindVariableFeatures) %>% 
  lapply(ScaleData, verbose = F) %>% lapply(RunPCA, verbose = F)
