...
obj_human = readRDS(multispecies_fn) %>% subset(Species == 'Human') %>% 
...
pdf(here(PLOTDIR, 'human_nac_integrated_subclusters.umap.pdf'), 
    height = 1.5, width = 1.5)
DimPlot(obj_human, group.by = 'integrated_subclusters', cols = cols2,
            raster = F, reduction = "umap.full", label = T, label.size = 1) & 
...
pdf(here(PLOTDIR, 'human_tshz1_pdyn_oprm1_drd.umap.pdf'), height = 3, width = 2)
FeaturePlot(obj_human, features = c('DRD1', 'DRD2', 'TSHZ1', 'PDYN', 'OPRM1', 'PENK'),
