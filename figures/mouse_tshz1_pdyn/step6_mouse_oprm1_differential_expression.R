...
obj_super$Oprm1 = getCounts(y)['Oprm1',]

design <- model.matrix(~ Oprm1 + Sex + size,  data = obj_super[[]])
...
df_deg = topTable(fit = vfit, coef = 'Oprm1', n = Inf) %>% 
...
## write out the DEGs
out_fn = here(TABLDIR, 'differential_expression_byOprm1_WMB-10Xv3-STR.NAc.xlsx')
...
pdf(here(PLOTDIR, 'oprm1_differential_expression.volcano.pdf'), 
...
ggplot(df_deg_sig, aes(x = logFC, y = -log10(Bonf.P.Val))) + 
  ...
  scale_color_manual(values = c('#238b45', '#114522')) +
  scale_shape_manual(values = c(20, 19)) +
  scale_alpha_manual(values = c(0.1, 1)) +
  scale_x_continuous(limits = c(0, NA)) +
  labs(x = 'Log2(Correlation with Oprm1)', 
