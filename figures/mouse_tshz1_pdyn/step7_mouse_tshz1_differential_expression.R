...
obj_super$Tshz1 = getCounts(y)['Tshz1',]

design <- model.matrix(~ Tshz1 + Sex + size,  data = obj_super[[]])
...
df_deg = topTable(fit = vfit, coef = 'Tshz1', n = Inf) %>% 
...
## write out the DEGs
out_fn = here(TABLDIR, 'differential_expression_byTshz1_WMB-10Xv3-STR.NAc.xlsx')
...
pdf(here(PLOTDIR, 'tshz1_differential_expression.volcano.pdf'), 
...
ggplot(df_deg_sig, aes(x = logFC, y = -log10(Bonf.P.Val))) + 
  ...
  scale_color_manual(values = c('#b856d7', '#5c2b6b')) +
  scale_shape_manual(values = c(20, 19)) +
  scale_alpha_manual(values = c(0.1, 1)) +
  labs(x = 'Log2(Correlation with Tshz1)', 
